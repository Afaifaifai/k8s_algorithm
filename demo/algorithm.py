...
class GeneticAlgorithm:
    ...
    def genetic_algorithm(self):
        ...
        for iter in range(self.max_iter):
            ...
            for p_index in range(self.particle_num):  # 解的數量
                for dim in range(self.dim):
            ...
            self.fitness_values = np.array([ # 算每個解的標準差總和
                np.sum ([
                    np.std( # 標準差
                        (np.array(fitness[dim]) / np.array(self.max_weight[dim])).tolist(), 
                        ddof=0
                    ) 
                    for dim in range(self.dim)
                ]) 
                for fitness in knapsack_fitness_values # fitness = [ dim [knapsack] ]
            ])
            # L = [[]]
            # print(L)
            ...
            fitness_values = np.array([  # 算所有新的解的標準差總和
                np.sum([
                    np.std(
                        (np.array(fitness[dim]) / np.array(self.max_weight[dim])).tolist(), 
                        ddof=0
                ...
                for fitness in new_knapsack_fitness_values
            ])
            
            idx_to_keep = np.argsort(fitness_values)[:self.particle_num] # sort後取前particle_num個解 PS: particle_num為一開始的解的數量
